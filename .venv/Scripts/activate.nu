...
export-env {
    ...
    let virtual_env = r#'C:\Users\Grigoriy_Klimanov\PycharmProjects\OOP_14_1_homework\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'oop-14-1-homework-py3.14'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'oop-14-1-homework-py3.14'#
    }
